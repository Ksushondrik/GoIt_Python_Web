...
async def search_contacts(first_name: Optional[str] = Query(None), last_name: Optional[str] = Query(None), email: Optional[str] = Query(None), db:AsyncSession = Depends(get_db)):
...
async def get_birthdays_upcoming(db: AsyncSession = Depends(get_db)):
    contacts = await repositories_contacts.get_birthdays_upcoming(db)
    if contacts is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="NOT FOUND!")
    return contacts
